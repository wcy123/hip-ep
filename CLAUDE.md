...
- **Qwen 3.5 vision cosine 0.48 — bisect summary**. The test `test_vision_grid_small_matches_cpu_cosine` fails with deterministic cos=0.482351; standalone EP output has every row 1..31 collapsed to the same prefix `~(0.33, -3.05, 0.33)` while CPU produces 32 distinct rows. **The bisect tool localised that pinning ANY intermediate value as a model output suppresses the bug** (cos=1.0 max_abs=0 bit-identical). Not just rope-chain values — pinning `cat_2` (rotary cos/sin embedding chain) also fixes it; pinning the very early `add_92` (rope position-encoding chain) fixes it; pinning later layer-norm / mul i64 values does NOT fix it. The 4 "fires `[elementwise-empty] Add(big=(128,1152), empty=(0,1152))`" we see in standalone become 0 firings in pinned — pinning prevents an upstream chain from collapsing to a 0-extent buffer. **What we ruled out this session**: (1) pool aliasing (forced `lifetimesOverlap = true` always — cos still 0.482351), (2) `hip_device_free` recycling (no-op'd — cos still 0.482351), (3) main_graph/loop-body pool sharing (the depth-pool fix above — cos still 0.482351), (4) shared integer-broadcast workspace reuse across rope-chain Adds (replaced with per-call `hipMalloc`+sync+`hipFree` — cos still 0.482351), (5) ORT graph optimisation level (DISABLE_ALL gives NaN cos=0.474, BASIC/EXTENDED/ALL all give cos=0.482351). The IR diff between standalone and add_92-pinned (~/tmp/p_standalone.mlir vs /tmp/p_pinned.mlir, after PoolAllocs on main_graph) shows pinned has 138 more `memref.global` constants, 7 fewer `hip.slice` ops, and the four rope-chain `hip.add` ops are structurally identical except add_92 writes to `%arg4` (out-param) in pinned vs `%alloc_326` (pool) in standalone. **What we have NOT tried**: dumping the runtime value of `add_92`'s output buffer (suspect: in standalone it's all-zeros or all-the-same-index, which would explain "Gather always returns row 0 of pos_embed_table" symptom); looking at the After-CSE / After-Canonicalize IR diff (the standalone has 14 more `arith.cmpi` and 21 more `arith.maxsi` than pinned, suggesting canonicalization decisions differ); rewriting the rope chain's broadcast Adds to NOT use the shared workspace (route through standard MIOpen instead of integer fallback); auditing `BufferViewFlowAnalysis::resolve` for whether it correctly tracks `memref.expand_shape` aliases of an alloc that gets re-used (the `alloc_321` two-cast pattern in the IR).
- **`Slice` with runtime extent 0 produces a TRUE dim-0 buffer, NOT a data-dim upper bound** (`lib/Conversion/OnnxToHip/SliceConversion.cpp::SliceToHip`, `lib/Runtime/real/elementwise.cpp::wrap_miopenOpTensor` empty-operand identity-copy). Earlier code wrapped runtime extents in `select(extent==0, upper_bound, extent)` to keep downstream MIOpen broadcast descriptors happy. That silently over-allocated the empty-slice output to the data dim — catastrophic for the `Slice(x, k, k, axis) -> Loop` pattern (the empty placeholder used as the Loop accumulator's v_init): inside the loop body, `tensor.dim(acc, axis)` returned the upper bound instead of 0, so the Concat-grown accumulator started at `upper_bound + chunk_size` and the downstream Reshape chain operated on a wildly oversized buffer. Canonical site: Qwen 3.5 vision encoder's per-block windowed-attention accumulator. Fix: SliceConversion now emits the true logical extent (including 0) for every path; `wrap_miopenOpTensor` detects any dim 0 on lhs/rhs/out, treats the empty operand as an additive/multiplicative identity, and `hipMemcpyAsync`s the non-empty operand into OUT (output type-inference sized OUT to `max(lhs, rhs)` per axis, so this is the only consistent answer when the empty side has 0 against the non-empty side's N > 1 dim). LLM `past_kv` empty slices on first decode token continue to work because downstream Concat/Reshape short-circuit naturally on dim 0. **Residual finding (handoff): the Qwen 3.5 vision encoder cosine bisect shows the bug is in the rope position-encoding chain: pinning ANY rope-chain intermediate (`add_92`, `embedding_2`, `view_4`, `mul_144`, `add_206`, `add_191`, `add_221`) as a model output yields cos=1.0 bit-identical (max_abs=0). Pinning out-of-chain values (`mul_2803`, `layer_norm_5`) leaves cos=0.48. Standalone (the failing test) produces an `image_features` output where every row 1..31 has the same prefix `~(0.33, -3.05, 0.33, ...)` — the rope chain is collapsing to a position-uniform value (consistent with `Gather` always returning row 0 of `pos_embed_table`). Three suspects ruled out: pool aliasing (forced all `lifetimesOverlap=true` → cos still 0.48), `hip_device_free` recycling (no-op'd → cos still 0.48), and main/loop pool sharing (depth-pool fix → cos still 0.48). Standalone also fires 4× `[elementwise-empty]` `Add(big=(128,1152), empty=(0,1152))` while pinned fires 0× — pinning an upstream value prevents an intermediate from going empty. Bisect tool (single-pass-multi-output) is in place to keep iterating. Likely next direction: dump the after-CSE / after-canonicalize IR around the rope chain for standalone vs pinned and find what CSE/canonicalize folds away in standalone that should stay; OR add a runtime trace that dumps the value of `add_92`'s memref right after its kernel runs (suspect: garbage / zero buffer indicates the kernel never actually wrote it).**
